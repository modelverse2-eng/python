...
def is_path_exists(player=1):
    """检查指定玩家是否存在到终点的可达路径（BFS）"""
    start = (player1_row, player1_col) if player == 1 else (player2_row, player2_col)
    visited = set()
    queue = deque([start])
    visited.add(start)
...
def generate_map():
    ...
    while True:
        generate_obstacles()
        generate_goal()
        if is_path_exists(player=1) and is_path_exists(player=2):
            break
...
def draw_board():
    ...
    canvas.create_oval(player1_col*CELL_SIZE+CELL_SIZE/2-radius,
