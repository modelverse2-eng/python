...
def update_records_display():
    if records:
        top5 = sorted(records.items(), key=lambda x: x[1])[:5] #按用时从小到大排序字典，取前 5 名
        text = "排行榜 (前5):\n" + "\n".join(f"{i+1}. {name} - {t:.2f} 秒" for i, (name, t) in enumerate(top5))#给列表加上索引
    else:#f"{i+1}. {name} - {t:.2f} 秒" → 格式化每行  #n".join(...) → 多行拼接成一个字符串
        text = "排行榜为空"
    records_label.config(text=text) #justify → 文本对齐方式 #font → 字体和大小
...
def generate_map():
    ...
    # 生成可达终点
    for _ in range(100):# 尝试100次生成终点不行就放弃
