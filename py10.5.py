import tkinter as tk
import tkinter.messagebox
import tkinter.simpledialog
import random
import time
import os
from collections import deque

# ---------------- 游戏参数 ----------------
CELL_SIZE = 50
ROWS, COLS = 10, 10
OBSTACLE_COUNT = 20
RECORDS_FILE = "records.txt"

# ---------------- 游戏状态 ----------------
player_row, player_col = 0, 0
goal_row, goal_col = None, None
obstacles = []
start_time = None
records = {}  # 字典: { "玩家名": 用时 }

# ---------------- 排行榜功能 ----------------
def load_records():
    global records
    records = {}
    if os.path.exists(RECORDS_FILE):
        with open(RECORDS_FILE, "r") as f:
            for line in f:
                parts = line.strip().split(",")
                if len(parts) == 2: #检查拆分后的列表长度是否为 2，防止文件格式错误导致报错
                    name, t = parts #将列表解包成两个变量
                    records[name] = float(t) #将玩家名字作为字典 key，用时转成浮点数作为 value 存入字典

def save_records():
    with open(RECORDS_FILE, "w") as f:
        for name, t in records.items():
            f.write(f"{name},{t:.2f}\n")

def add_record(elapsed):
    global records
    name = tk.simpledialog.askstring("记录名字", "请输入你的名字：")
    if not name:
        name = "匿名玩家"
    # 保留最好成绩
    if name in records:
        records[name] = min(records[name], elapsed) #如果字典中已有该玩家的记录，则保留最好成绩
    else:
        records[name] = elapsed #如果玩家名字不存在，直接添加新的记录
    save_records()
    update_records_display() #保存后更新排行榜

def update_records_display():
    if records:
        # 将字典 items 转为列表 [(name, t), ...]
        items = list(records.items())

        # ---------- 手写排序: 冒泡排序按用时从小到大 ----------   比较+交换
        n = len(items)
        for i in range(n): #外层循环，用于控制比较轮数
            for j in range(0, n-i-1): #内层循环，控制每轮比较次数
                if items[j][1] > items[j+1][1]:  # 如果前面的用时大于后面的，用于交换
                    items[j], items[j+1] = items[j+1], items[j] #交换两者位置，让用时大的那个“往后冒”
        # ---------- 排序完成 ----------

        # 取前5名
        top5 = items[:5]

        # 拼接排行榜文本
        text = "排行榜 (前5):\n" + "\n".join(f"{i+1}. {name} - {t:.2f} 秒" for i, (name, t) in enumerate(top5))
    else:
        text = "排行榜为空"
    
    # 更新显示
    records_label.config(text=text)

# ---------------- 游戏逻辑 ----------------
def find_path(start, goal):
    """BFS 寻路（最短路径）"""
    q, visited, parent = deque([start]), {start}, {start: None}
    while q:
        r, c = q.popleft()
        if (r, c) == goal:
            path = []
            cur = (r, c)
            while cur:
                path.append(cur)
                cur = parent[cur]
            return path[::-1]
        for dr, dc in [(-1,0),(1,0),(0,-1),(0,1)]:
            nr, nc = r+dr, c+dc
            if 0<=nr<ROWS and 0<=nc<COLS and (nr,nc) not in obstacles and (nr,nc) not in visited:
                visited.add((nr,nc))
                parent[(nr,nc)] = (r,c)
                q.append((nr,nc))
    return None

def generate_map():
    """生成障碍物和合法终点"""
    global obstacles, goal_row, goal_col #声明全局变量
    obstacles = [] #障碍物集合
    while len(obstacles) < OBSTACLE_COUNT: #这里的len是障碍物列表长度 #OBSTACLE_COUNT目标障碍物数量
        r, c = random.randint(0, ROWS-1), random.randint(0, COLS-1) #障碍物的X Y 值随机
        if (r, c) != (player_row, player_col) and (r, c) not in obstacles: #不能让障碍物生成在出生点
            obstacles.append((r, c)) #添加到列表以防生成重复
    # 生成可达终点
    for _ in range(1000):# 尝试1000次生成终点不行就放弃
        r, c = random.randint(0, ROWS-1), random.randint(0, COLS-1) #生成随机终点
        if (r, c) != (player_row, player_col) and (r, c) not in obstacles: #检查候选终点是否合法
            if find_path((player_row, player_col), (r, c)):
                goal_row, goal_col = r, c
                return #检查是否可达  从起点（pc pr）到候选终点
    goal_row, goal_col = ROWS-1, COLS-1  # 默认终点：如果有路径（find_path 返回非空），就把 (r, c) 设为终点

def draw_board():
    canvas.delete("all")
    for r in range(ROWS):
        for c in range(COLS):
            x1, y1 = c*CELL_SIZE, r*CELL_SIZE
            x2, y2 = x1+CELL_SIZE, y1+CELL_SIZE
            fill = "white"
            if (r,c) in obstacles: fill="black"
            elif (r,c)==(goal_row,goal_col): fill="green"
            elif (r,c)==(player_row,player_col): fill="red"
            canvas.create_rectangle(x1,y1,x2,y2,fill=fill,outline="gray")

def game_victory(msg):
    global start_time
    if start_time:
        elapsed = time.time()-start_time
        add_record(elapsed)
        tk.messagebox.showinfo("胜利", f"{msg}\n用时 {elapsed:.2f} 秒！")
    else:
        tk.messagebox.showinfo("胜利", msg)

def move_player(dr, dc):
    global player_row, player_col
    nr, nc = player_row+dr, player_col+dc
    if 0<=nr<ROWS and 0<=nc<COLS and (nr,nc) not in obstacles:
        player_row, player_col = nr, nc
        draw_board()
        if (nr,nc)==(goal_row,goal_col):
            game_victory("手动到达终点！")

def auto_move():
    route = find_path((player_row, player_col), (goal_row, goal_col))
    if not route:
        tk.messagebox.showinfo("提示","没有路径可到终点！")
        return
    def step(i):
        global player_row, player_col
        if i < len(route):
            player_row, player_col = route[i]
            draw_board()
            if (player_row,player_col) == (goal_row,goal_col):
                game_victory("自动寻路完成！")
            else:
                root.after(300, step, i+1)
    step(0)

def restart_game():
    global player_row, player_col, start_time  # 修正拼写错误
    player_row, player_col = 0,0
    generate_map()
    draw_board()
    start_time = time.time()  # 记录游戏开始时间，用于计算用时

# ---------------- UI ----------------
root = tk.Tk()
root.title("迷宫寻路游戏")

frame = tk.Frame(root)
frame.pack()

canvas = tk.Canvas(frame,width=COLS*CELL_SIZE,height=ROWS*CELL_SIZE)
canvas.grid(row=0,column=0)

records_label = tk.Label(frame,text="排行榜为空",justify="left",font=("Arial",12))
records_label.grid(row=0,column=1,padx=20,sticky="n")

tk.Button(root,text="自动寻路",command=auto_move).pack(fill="x")
tk.Button(root,text="重新开始",command=restart_game).pack(fill="x")

root.bind("<Up>",lambda e: move_player(-1,0))
root.bind("<Down>",lambda e: move_player(1,0))
root.bind("<Left>",lambda e: move_player(0,-1))
root.bind("<Right>",lambda e: move_player(0,1))

load_records()
update_records_display()
restart_game()
root.mainloop()
