...
# 随机数据生成函数
def generate_random_data(data_type="int", size=10, start=0, end=1145141919810):
...
# 点击按钮时更新显示的数据
def update_data():
    groups = 11       # 要生成多少组随机数据
    group_size = 45141 # 每组里有多少个数据
    results = []      # 用来存放最终显示结果的列表
    
    for i in range(groups):  # 循环生成每一组数据，重复同一操作，只是针对不同的元素
        data = generate_random_data("int", size=group_size, start=1, end=11451419019810)  # 生成一组随机整数
        
        start_time = time.perf_counter()         # 记录排序前的时间（高精度计时）
        sorted_data = sorted(data, reverse=True) # 用 sorted 函数排序，并设置 reverse=True 让结果是降序
        end_time = time.perf_counter()           # 记录排序后的时间
        
        duration = (end_time - start_time) * 1000  # 计算排序耗时，单位换成毫秒
        # 把每组的前10个数据和排序用时拼成字符串，放到 results 列表里
        results.append(f"第{i+1}组: {sorted_data[:1]}... 用时 {duration:.4f} ms")
...
root.title("随机数据生成器（降序 + 计时）")  # 设置窗口标题
